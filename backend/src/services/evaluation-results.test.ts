...
function completePieces(): RunPieces {
  return {
...
test("buildEvaluationResults includes every section for a complete run", () => {
  const card = buildEvaluationResults(completePieces(), "2026-07-09T00:00:00.000Z");
  assert.equal(card.bioeval.resultsVersion, "1.0");
...
test("buildEvaluationResults nulls optional config when absent", () => {
  const pieces = completePieces();
  pieces.parsingFunctions = null;
  pieces.llmJudgeConfig = null;
  const card = buildEvaluationResults(pieces, "2026-07-09T00:00:00.000Z");
  assert.equal(card.postprocessing, null);
  assert.equal(card.metrics.llmJudge, null);
});

test("buildEvaluationResults throws when the run is not complete", () => {
  const pieces = completePieces();
  pieces.status = "processing";
  assert.throws(
    () => buildEvaluationResults(pieces, "2026-07-09T00:00:00.000Z"),
    EvaluationResultsIncompleteError
  );
});
