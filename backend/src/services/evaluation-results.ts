export type RunPieces = {
  evaluationId: string;
  inferenceId: string;
  datasetId: string;
  status: string;
  meta: {
    // fields buildEvaluationResults reads:
    model: string;
...
export type EvaluationResults = {
  bioeval: { resultsVersion: string; exportedAt: string };
  evaluation: {
    evaluationId: string;
    inferenceId: string;
    datasetId: string;
...
export class EvaluationResultsIncompleteError extends Error {
  constructor(message = "Evaluation is not complete; cannot build results") {
    super(message);
    this.name = "EvaluationResultsIncompleteError";
  }
}

export function buildEvaluationResults(
  p: RunPieces,
  exportedAt: string
): EvaluationResults {
  if (p.status !== "done") {
    throw new EvaluationResultsIncompleteError();
  }
  return {
    bioeval: { resultsVersion: "1.0", exportedAt },
    evaluation: {
