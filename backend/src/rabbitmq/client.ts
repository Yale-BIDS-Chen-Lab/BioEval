...
class RabbitMQClient {
  ...
  // Overridable so tests can use a short timeout instead of waiting 5s.
  ...
  async connect(): Promise<void> {
    ...
    // A confirm channel lets us await broker acknowledgement of each publish,
    // so a dropped message surfaces to the caller instead of vanishing.
  ...
  private async publishWithConfirm(queue: string, body: string): Promise<void> {
    ...
    const timeoutMs = this.publishConfirmTimeoutMs;
    return new Promise<void>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `rabbitmq: publish to '${queue}' timed out after ${timeoutMs}ms`
          )
        );
      }, timeoutMs);
