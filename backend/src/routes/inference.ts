...
router.post(
  ...
  ...validatedRoute(
    ...
    async (req, res) => {
      ...
      // Phase 1: validate ALL models before any DB write, so a later model's
      // validation error can't leave earlier models partially committed.
      ...
      // Phase 2: persist + publish. Each row is committed before publish; if a
      // publish fails the row is marked 'failed' so the user sees it instead of
      // a silent stuck-pending row. (We deliberately do NOT auto re-publish
      // stale pending rows on startup — that mass re-ran the historical backlog.)
    ...
    async (req, res) => {
      ...
      try {
        ...
        await markInferenceFailed(newInferenceId).catch((dbErr) =>
          console.error(`also failed to mark ${newInferenceId} as failed:`, dbErr)
        );
        return res.status(StatusCodes.BAD_GATEWAY).json({
          success: false,
          inferenceId: newInferenceId,
